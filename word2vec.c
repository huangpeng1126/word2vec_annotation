...
//cn,该本词出现的数量
//point,哈弗曼树中，从root节点到该基本词的路径，指针数组,存放的是每个父节点在哈夫曼树中的编码——哈夫曼编码：[0, vocab_size-1]
//叶子节点从大到小编号；[vocab_size,vocab_size*2-1)，中间节点从小到大编号
//word,基本词字面
//code,该基本词的哈弗曼码
...
int *vocab_hash;
//@brief
//vocab_size保存字典大小（不重复word个数）
//layer1_size 第一层特征空间大小，通常远远小于字典大小，可以近似认为是word到隐空间映射的维度
...
//@brief 哈弗曼编码，
//流程是先在所有的vacabulary中找2个最小weight的节点作为叶子节点，weight即基本词出现的次数cn,合并成一个父亲节点，
//从词序列中剔除这两个节点，并加入合成的父亲节点，重复上述过程，建成一颗哈弗曼树，最长路径log|V|最小的二叉树
...
void CreateBinaryTree() {
  ...
  // NOTE：	下面的算法基于一个前提：所有的叶子节点已经按照从大到小排好序，下标是[0, vocab_size)。每次从最后一个叶子节点取出最小的叶子节点；另外，中间节点（非叶子结点）
  //		则按照从小打大顺序排列，下标是[vocab_size,vocab_size*2)；
  // NOTE：a循环范围是(0, vocab_size-1)，表示总共n-2个中间节点（n表示字典大小，等于vocab_size）
  for (a = 0; a < vocab_size - 1; a++) {
    ...
    //较大的叶子节点编码为1；binary使用call分配，初始化为0，所以省略了一个赋值操作：binary[min1i] = 0，很容易引起误解。
    //binary[min1i] = 0;
    binary[min2i] = 1;
  }
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        //word2vec里处理输入向量的方式和Hilton论文里神经网络不同，前者是将所有的上下文向量加起来，而Hilton论文里是将上下文向量拼接起来形成一个 N*m长度的向量（N是上下文词个数）
        for (c = 0; c < layer1_size; c++) neu1[c] += syn0[c + last_word * layer1_size];
        cw++;
      }
      if (cw) {
        for (c = 0; c < layer1_size; c++) neu1[c] /= cw;	//和论文里不一致，代码里是输入向量求和算平均值
